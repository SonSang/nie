...
def triangle_aspect_ratio(points: th.Tensor, faces: th.Tensor):
    ...
    face_height_0 = th.norm(th.cross(face_edge_dir_0, face_edge_dir_1), dim=-1)
    face_height_1 = th.norm(th.cross(face_edge_dir_1, face_edge_dir_2), dim=-1)
    face_height_2 = th.norm(th.cross(face_edge_dir_2, face_edge_dir_0), dim=-1)

    face_height_0 = face_height_0 / (face_edge_len_0 + 1e-6)
    face_height_1 = face_height_1 / (face_edge_len_1 + 1e-6)
    face_height_2 = face_height_2 / (face_edge_len_2 + 1e-6)

    ar_0 = face_edge_len_0 / (face_height_0 + 1e-6)
    ar_1 = face_edge_len_1 / (face_height_1 + 1e-6)
    ar_2 = face_edge_len_2 / (face_height_2 + 1e-6)

    ar = th.max(th.stack([ar_0, ar_1, ar_2], dim=-1), dim=-1)[0]
