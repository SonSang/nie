...
class AlphaRenderer(NormalsRenderer):
    ...
    * shillouette
    ...
    def forward_alpha(self,
        ...
        '''
        @ verts: Positions of vertices in the object space. [# point, 3]
        @ vnormals: Per-vertex normal vectors in the object space. [# point, 3]
        @ faces: [# tri, 3]
        @ faces_alpha: [# tri]
        @ max_num_peeling: Maximum number of depth peelings.
        @ min_transmittance: If transmittance goes below this value, the peeling stops.
        '''
        ...
        '''        
        # (x,y,z) -> (x,y,z,1)
        verts_hom = th.cat([verts, th.ones_like(verts[:, [0]])], axis=1)

        # [# batch, # point, 4]
        verts_clip = verts_hom @ self._mvp.transpose(-2,-1)

        '''
        Vnormals
        '''
        # (x,y,z) -> (x,y,z,1)
        vnormals_hom = th.cat([vnormals, th.zeros_like(vnormals[:, [0]])], axis=1)
        # [# batch, # point, 4]
        vnormals_mv = vnormals_hom @ self._mv.transpose(-2,-1)
        # [# batch, # point, 3]
        vnormals_mv = vnormals_mv[..., :3]
        # in the view space, normals should be oriented toward viewer, 
        # so the z coordinates should be negative;
        vnormals_mv[vnormals_mv[:, :, 2] > 0.] = -vnormals_mv[vnormals_mv[:, :, 2] > 0.]
        vnormals_mv = vnormals_mv.contiguous()

        '''
        Lightdirs
        '''
        lightdirs_mv = th.tensor([LIGHT_DIR], dtype=th.float32, device=verts.device) # [1, 3]

        '''
        Depth peeling for rendering
        '''
        num_peeling = 0
        num_batch = verts_clip.shape[0]
        resolution = self._image_size[0]
        transmittance = th.ones((num_batch, resolution, resolution), dtype=th.float32, device=verts.device)
        f_diffuse = th.zeros((num_batch, resolution, resolution, 1), dtype=th.float32, device=verts.device)
        f_depth = th.zeros((num_batch, resolution, resolution, 1), dtype=th.float32, device=verts.device)

        with dr.DepthPeeler(self._glctx, verts_clip, faces, resolution=self._image_size) as peeler:
            for pi in range(max_num_peeling):
                # [# batch, # res, # res, 4]
                rast_out, _ = peeler.rasterize_next_layer()

                tri_idx = rast_out[..., -1]    # id of triangle for each pixel;

                '''
                Diffuse
                '''
                # [# batch, # res, # res, 3]
                pixel_normal_mv, _ = dr.interpolate(vnormals_mv, rast_out, faces)
                # normalize normal;
                pixel_normal_mv = pixel_normal_mv / th.clamp(th.norm(pixel_normal_mv, p=2, dim=-1, keepdim=True), min=1e-5)
                # [# batch, # res, # res, 1]
                diffuse = th.sum(lightdirs_mv.view(-1, 1, 1, 3) * pixel_normal_mv, -1, keepdim=True)
                diffuse_aa = dr.antialias(
                    diffuse,
                    rast_out, 
                    verts_clip, 
                    faces
                ...
                Aggregate
                '''
                valid_tri_idx = th.where(tri_idx > 0)
                valid_tri_idx_alpha = faces_alpha[tri_idx[valid_tri_idx].to(dtype=th.long) - 1]

                # [# batch, # res, # res]
                curr_alpha = th.ones_like(transmittance)
                curr_alpha[valid_tri_idx] = valid_tri_idx_alpha
                
                # alpha blending;
                f_diffuse = f_diffuse + ((transmittance * curr_alpha).unsqueeze(-1) * diffuse_aa)
                f_depth = f_depth + ((transmittance * curr_alpha).unsqueeze(-1) * depth_aa)
                transmittance = transmittance * (1. - curr_alpha)

                num_peeling += 1

                if th.all(transmittance < min_transmittance):
                    break

        return f_diffuse, f_depth, num_peeling
